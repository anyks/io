...
private:
  struct TimerCtx { IocpEngine* self; socket_t fd; };
  struct ListenerTimerCtx { IocpEngine* self; socket_t listen_fd; };
  struct SockState {
    char* buf{nullptr};
    size_t buf_size{0};
...
bool IocpEngine::init(const NetCallbacks& cbs) {
  ...
  if (tmp != INVALID_SOCKET) {
    DWORD bytes = 0; GUID guid = WSAID_CONNECTEX;
    if (WSAIoctl(tmp, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(guid), &ConnectEx_, sizeof(ConnectEx_), &bytes, NULL, NULL) != 0) {
      IO_LOG_ERR("WSAIoctl(GET ConnectEx) failed: WSA=%d", WSAGetLastError());
    }
    GUID guid2 = WSAID_ACCEPTEX;
    if (WSAIoctl(tmp, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid2, sizeof(guid2), &AcceptEx_, sizeof(AcceptEx_), &bytes, NULL, NULL) != 0) {
...
void IocpEngine::destroy() {
  // Phase 1: cancel all outstanding I/O before stopping the worker
  {
    ...
    for (const auto& kv : sockets_) {
      ...
      if (kv.second.timer) { DeleteTimerQueueTimer(timer_queue_, kv.second.timer, INVALID_HANDLE_VALUE); }
      if (kv.second.timer_ctx) { delete kv.second.timer_ctx; }
    }
    // cancel autotune timers now to avoid callbacks during teardown
    for (auto& kv : autotune_timer_) {
      if (kv.second) { DeleteTimerQueueTimer(timer_queue_, kv.second, INVALID_HANDLE_VALUE); }
    }
  }
  ...
  if (worker_.joinable()) worker_.join();
  {
    ...
    for (auto& kv : sockets_) { closesocket((SOCKET)kv.first); }
    for (auto lfd : listeners_) { closesocket((SOCKET)lfd); }
    sockets_.clear(); listeners_.clear();
    autotune_timer_.clear(); autotune_cfg_.clear(); accepted_in_window_.clear();
  }
  if (timer_queue_) { DeleteTimerQueueEx(timer_queue_, INVALID_HANDLE_VALUE); timer_queue_ = nullptr; }
  if (iocp_) { CloseHandle(iocp_); iocp_ = nullptr; }
  if (winsock_inited_) { WSACleanup(); winsock_inited_ = false; }
}
...
bool IocpEngine::add_socket(socket_t fd, char* buffer, size_t buffer_size, ReadCallback cb) {
  ...
  if (!h) { IO_LOG_ERR("CreateIoCompletionPort(sock=%d) failed: GLE=%lu", (int)fd, GetLastError()); return false; }
  u_long nb = 1; ioctlsocket((SOCKET)fd, FIONBIO, &nb);
  std::lock_guard<std::mutex> lk(mtx_);
  SockState st{}; st.buf = buffer; st.buf_size = buffer_size; st.read_cb = std::move(cb);
...
bool IocpEngine::delete_socket(socket_t fd) {
  ...
  if (it != sockets_.end()) {
    ...
    if (it->second.timer) { DeleteTimerQueueTimer(timer_queue_, it->second.timer, INVALID_HANDLE_VALUE); it->second.timer = nullptr; }
    if (it->second.timer_ctx) { delete it->second.timer_ctx; it->second.timer_ctx = nullptr; }
    sockets_.erase(it);
  }
  return true;
}
...
bool IocpEngine::connect(socket_t fd, const char* host, uint16_t port, bool async) {
  ...
  if (InetPtonA(AF_INET, host, &addr.sin_addr) != 1) return false;
  if (!async || !ConnectEx_) {
    u_long nb0 = 0; ioctlsocket((SOCKET)fd, FIONBIO, &nb0);
    int r = ::connect((SOCKET)fd, (sockaddr*)&addr, sizeof(addr));
    u_long nb1 = 1; ioctlsocket((SOCKET)fd, FIONBIO, &nb1);
    if (r == 0) { if (cbs_.on_accept) cbs_.on_accept(fd); post_read(fd); return true; }
    return false;
  }
  ...
  PerIo* p = new PerIo(); p->op = OpType::Connect; p->fd = fd;
  BOOL ok = ConnectEx_((SOCKET)fd, (sockaddr*)&addr, sizeof(addr), NULL, 0, NULL, &p->ol);
  if (!ok) { int e = WSAGetLastError(); if (e != WSA_IO_PENDING) { IO_LOG_ERR("ConnectEx(fd=%d) failed immediately: WSA=%d", (int)fd, e); delete p; return false; } }
  return true;
}
...
bool IocpEngine::disconnect(socket_t fd) {
  ...
  shutdown((SOCKET)fd, SD_BOTH); closesocket((SOCKET)fd);
...
bool IocpEngine::accept(socket_t listen_socket, bool async, uint32_t max_connections) {
  (void)async; max_conn_ = max_connections;
  ...
  for (uint32_t i=0; i<depth; ++i) post_accept(listen_socket);
  return true;
}

bool IocpEngine::write(socket_t fd, const char* data, size_t data_size) {
  std::lock_guard<std::mutex> lk(mtx_);
  auto it = sockets_.find(fd); if (it == sockets_.end()) return false;
  auto& st = it->second; st.out_queue.insert(st.out_queue.end(), data, data + data_size);
  if (!st.sending) { st.sending = true; post_write_unlocked(fd, st); }
  return true;
}

bool IocpEngine::post(uint32_t val) {
  PerIo* p = new PerIo(); p->op = OpType::User; p->user_val = val;
  BOOL ok = PostQueuedCompletionStatus(iocp_, val, 0, &p->ol);
  if (!ok) { IO_LOG_ERR("PostQueuedCompletionStatus(user=%u) failed: GLE=%lu", val, GetLastError()); delete p; return false; }
  return true;
}
...
bool IocpEngine::set_read_timeout(socket_t fd, uint32_t timeout_ms) {
  ...
  if (!timer_queue_) timer_queue_ = CreateTimerQueue();
  if (it->second.timer) { ChangeTimerQueueTimer(timer_queue_, it->second.timer, timeout_ms, 0); }
  else {
    HANDLE hTimer = nullptr;
    auto* ctx = new TimerCtx{this, fd};
    CreateTimerQueueTimer(&hTimer, timer_queue_, &IocpEngine::on_socket_timer, ctx, timeout_ms, 0, WT_EXECUTEDEFAULT);
...
void IocpEngine::loop() {
  while (true) {
    ...
    if (!pol) {
      if (!ok) { IO_LOG_ERR("GetQueuedCompletionStatus error: GLE=%lu", GetLastError()); continue; }
      if (bytes==0 && key==0) { if (!running_) break; else continue; }
    }
    ...
    switch (p->op) {
      case OpType::User: {
        if (cbs_.on_user) cbs_.on_user(p->user_val);
        delete p; break;
      }
      case OpType::Accept: {
        ...
        if (!ok) {
          socket_t sock = p->fd;
          {
            ...
            if (outstanding_accepts_[listen_fd]>0) outstanding_accepts_[listen_fd]--;
          }
          if (sock != -1) closesocket((SOCKET)sock);
          uint32_t target=0, out=0; { std::lock_guard<std::mutex> lk(mtx_); target = target_accept_depth_[listen_fd]; out = outstanding_accepts_[listen_fd]; }
          delete p;
          if (running_ && out < target) post_accept(listen_fd);
          break;
        }
        socket_t newfd = p->fd; // accepted socket
        setsockopt((SOCKET)newfd, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char*)&accept_listen_, sizeof(accept_listen_));
        {
          std::lock_guard<std::mutex> lk(mtx_);
          auto &vec = pending_accepts_[listen_fd];
          vec.erase(std::remove(vec.begin(), vec.end(), p), vec.end());
          accepted_in_window_[listen_fd]++;
          if (max_conn_==0 || cur_conn_ < max_conn_) { cur_conn_++; }
          else { closesocket((SOCKET)newfd); newfd = -1; }
        }
        if (newfd != (socket_t)INVALID_SOCKET) { IO_LOG_DBG("accept: new socket=%p", (void*)(uintptr_t)newfd); if (cbs_.on_accept) cbs_.on_accept(newfd); }
        uint32_t target=0, out=0; { std::lock_guard<std::mutex> lk(mtx_); if (outstanding_accepts_[listen_fd]>0) outstanding_accepts_[listen_fd]--; target = target_accept_depth_[listen_fd]; out = outstanding_accepts_[listen_fd]; }
        if (out < target) post_accept(listen_fd);
        delete p; break;
      }
      case OpType::Connect: {
        socket_t fd = p->fd;
        if (!ok) {
          ...
          delete p; if (err == ERROR_OPERATION_ABORTED) break;
          if (cbs_.on_close) cbs_.on_close(fd); shutdown((SOCKET)fd, SD_BOTH); closesocket((SOCKET)fd);
          std::lock_guard<std::mutex> lk(mtx_); sockets_.erase(fd);
          break;
        }
        delete p;
      ...
      case OpType::Read: {
        ...
        if (!ok) { delete p; if (err == ERROR_OPERATION_ABORTED) break; if (cbs_.on_close) cbs_.on_close(fd); closesocket((SOCKET)fd); std::lock_guard<std::mutex> lk(mtx_); sockets_.erase(fd); if (cur_conn_>0) cur_conn_--; break; }
        if (bytes == 0) { delete p; if (cbs_.on_close) cbs_.on_close(fd); closesocket((SOCKET)fd); std::lock_guard<std::mutex> lk(mtx_); sockets_.erase(fd); if (cur_conn_>0) cur_conn_--; break; }
        ReadCallback cb; char* buf=nullptr; size_t sz=0; uint32_t tms=0; HANDLE timer=nullptr;
        {
          std::lock_guard<std::mutex> lk(mtx_); auto it = sockets_.find(fd);
          if (it != sockets_.end()) { cb = it->second.read_cb; buf = it->second.buf; sz = (size_t)bytes; tms = it->second.timeout_ms; timer = it->second.timer; }
        }
        if (cb) cb(fd, buf, sz);
        if (tms>0 && timer) { ChangeTimerQueueTimer(timer_queue_, timer, tms, 0); }
        post_read(fd);
        delete p; break;
      }
      case OpType::Write: {
        socket_t fd = p->fd; if (!ok) { delete p; if (err == ERROR_OPERATION_ABORTED) break; if (cbs_.on_close) cbs_.on_close(fd); closesocket((SOCKET)fd); std::lock_guard<std::mutex> lk(mtx_); sockets_.erase(fd); if (cur_conn_>0) cur_conn_--; break; }
        size_t wrote = bytes; delete p;
        if (cbs_.on_write) cbs_.on_write(fd, wrote);
        std::lock_guard<std::mutex> lk(mtx_);
        auto it = sockets_.find(fd); if (it != sockets_.end()) {
          if (wrote <= it->second.out_queue.size()) it->second.out_queue.erase(it->second.out_queue.begin(), it->second.out_queue.begin()+wrote);
          if (!it->second.out_queue.empty()) { post_write_unlocked(fd, it->second); }
          else { it->second.sending = false; }
        }
        break;
      }
    }
...
void IocpEngine::post_read(socket_t fd) {
  std::lock_guard<std::mutex> lk(mtx_);
  auto it = sockets_.find(fd); if (it == sockets_.end()) return; auto& st = it->second;
  if (st.paused) { return; }
  if (st.pending_read) { return; }
  PerIo* p = new PerIo(); p->op = OpType::Read; p->fd = fd; p->wbuf.buf = st.buf; p->wbuf.len = (ULONG)st.buf_size;
  DWORD flags = 0; DWORD recvd = 0;
  st.pending_read = p;
  int r = WSARecv((SOCKET)fd, &p->wbuf, 1, &recvd, &flags, &p->ol, NULL);
  if (r != 0) { int e = WSAGetLastError(); if (e != WSA_IO_PENDING) { IO_LOG_ERR("WSARecv(fd=%d) failed: WSA=%d", (int)fd, e); st.pending_read = nullptr; delete p; } }
}

void IocpEngine::post_write_unlocked(socket_t fd, SockState& st) {
  if (st.out_queue.empty()) return;
  PerIo* p = new PerIo(); p->op = OpType::Write; p->fd = fd;
  p->dynbuf.assign(st.out_queue.begin(), st.out_queue.end());
  p->wbuf.buf = p->dynbuf.data(); p->wbuf.len = (ULONG)p->dynbuf.size();
  DWORD sent = 0; int r = WSASend((SOCKET)fd, &p->wbuf, 1, &sent, 0, &p->ol, NULL);
  if (r != 0) { int e = WSAGetLastError(); if (e != WSA_IO_PENDING) { IO_LOG_ERR("WSASend(fd=%d) failed: WSA=%d", (int)fd, e); delete p; st.sending = false; } }
}
...
void IocpEngine::post_accept(socket_t listen_fd) {
  ...
  if (as == INVALID_SOCKET) return;
  CreateIoCompletionPort((HANDLE)as, iocp_, (ULONG_PTR)as, 0);
  PerIo* p = new PerIo(); p->op = OpType::Accept; p->fd = (socket_t)as; p->dynbuf.resize(2*(sizeof(sockaddr_storage)+16));
  DWORD bytes = 0;
  BOOL ok = FALSE;
  if (AcceptEx_) {
    ok = AcceptEx_((SOCKET)listen_fd, as, p->dynbuf.data(), 0, (DWORD)(sizeof(sockaddr_storage)+16), (DWORD)(sizeof(sockaddr_storage)+16), &bytes, &p->ol);
  }
  if (!ok) { int e = WSAGetLastError(); if (e != WSA_IO_PENDING) { IO_LOG_ERR("AcceptEx(listen=%d) failed: WSA=%d", (int)listen_fd, e); delete p; closesocket(as); return; } }
  accept_listen_ = (SOCKET)listen_fd;
  {
    std::lock_guard<std::mutex> lk(mtx_);
    outstanding_accepts_[listen_fd]++;
    pending_accepts_[listen_fd].push_back(p);
    IO_LOG_DBG("post_accept: listen=%p outstanding=%u", (void*)(uintptr_t)listen_fd, outstanding_accepts_[listen_fd]);
  }
}
...
bool IocpEngine::set_accept_depth(socket_t listen_socket, uint32_t depth) {
  ...
  for (uint32_t i=0; i<to_post; ++i) post_accept(listen_socket);
  return true;
}
...
bool IocpEngine::set_accept_autotune(socket_t listen_socket, const AcceptAutotuneConfig& cfg) {
  ...
  if (it == autotune_timer_.end()) {
    HANDLE hTimer = nullptr; auto* ctx = new ListenerTimerCtx{this, listen_socket};
    CreateTimerQueueTimer(&hTimer, timer_queue_, &IocpEngine::on_autotune_timer, ctx, cfg.window_ms, cfg.window_ms, WT_EXECUTEDEFAULT);
...
VOID CALLBACK IocpEngine::on_socket_timer(PVOID param, BOOLEAN) {
  ...
  IocpEngine* self = ctx->self; socket_t fd = ctx->fd;
  {
    std::lock_guard<std::mutex> lk(self->mtx_);
    auto it = self->sockets_.find(fd);
    if (it != self->sockets_.end()) { it->second.timer = nullptr; if (it->second.timer_ctx) { delete it->second.timer_ctx; it->second.timer_ctx = nullptr; } }
  }
  if (self->cbs_.on_close) self->cbs_.on_close(fd);
  shutdown((SOCKET)fd, SD_BOTH); closesocket((SOCKET)fd);
  {
    std::lock_guard<std::mutex> lk2(self->mtx_);
    self->sockets_.erase(fd);
    if (self->cur_conn_>0) self->cur_conn_--;
  }
}

VOID CALLBACK IocpEngine::on_autotune_timer(PVOID param, BOOLEAN) {
  auto* ctx = static_cast<ListenerTimerCtx*>(param);
  IocpEngine* self = ctx->self; socket_t lfd = ctx->listen_fd;
  AcceptAutotuneConfig cfg{}; uint32_t accepted = 0; uint32_t curDepth = 0;
  {
    std::lock_guard<std::mutex> lk(self->mtx_);
    if (self->autotune_cfg_.find(lfd) == self->autotune_cfg_.end()) return;
    cfg = self->autotune_cfg_[lfd];
    if (!cfg.enabled) return;
    if (self->listeners_.find(lfd) == self->listeners_.end()) return;
  ...
  if (accepted >= cfg.high_watermark) {
    ...
    if (curDepth > cfg.min_depth) next = std::max<uint32_t>(cfg.min_depth, curDepth - cfg.down_step);
  }
  if (next != curDepth) {
