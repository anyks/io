...
public:
  ...
  ~IouringEngine() override { destroy(); }

  bool init(const NetCallbacks& cbs) override {
    cbs_ = cbs;
    if (io_uring_queue_init(256, &ring_, 0) != 0) return false;
  ...
  void destroy() override {
    ...
    if (user_efd_ != -1) { uint64_t one = 1; (void)::write(user_efd_, &one, sizeof(one)); }
  ...
  bool set_accept_depth(socket_t listen_socket, uint32_t depth) override {
    (void)listen_socket; (void)depth; return true;
  }
  ...
  bool delete_socket(socket_t fd) override {
    ...
    if (it != sockets_.end()) {
      if (it->second.timeout_armed) { submit_timeout_remove(fd, it->second.timeout_key); it->second.timeout_armed = false; it->second.timeout_key = 0; }
      sockets_.erase(it);
    }
    return true;
  }
  ...
  bool connect(socket_t fd, const char* host, uint16_t port, bool async) override {
    ...
    {
      std::lock_guard<std::mutex> lk(mtx_);
      if (sockets_.find(fd) == sockets_.end()) return false;
    }
    ...
    if (!async) {
      ...
      if (r == 0) { if (cbs_.on_accept) cbs_.on_accept(fd); submit_recv(fd); return true; }
      return false;
    }
    ...
    {
      std::lock_guard<std::mutex> lk(mtx_);
      auto it = sockets_.find(fd); if (it != sockets_.end()) it->second.connecting = true;
    }
    return true;
  }
  ...
  bool disconnect(socket_t fd) override {
    if (fd >= 0) {
      // cancel timer if any
      {
        std::lock_guard<std::mutex> lk(mtx_);
        auto it = sockets_.find(fd);
        if (it != sockets_.end() && it->second.timeout_armed) { submit_timeout_remove(fd, it->second.timeout_key); it->second.timeout_armed = false; it->second.timeout_key = 0; }
      }
      ::shutdown(fd, SHUT_RDWR); ::close(fd); if (cbs_.on_close) cbs_.on_close(fd);
    }
    return true;
  }

  bool accept(socket_t listen_socket, bool async, uint32_t max_connections) override {
    if (async) { int flags = fcntl(listen_socket, F_GETFL, 0); if (flags < 0) flags = 0; fcntl(listen_socket, F_SETFL, flags | O_NONBLOCK); }
  ...
  bool write(socket_t fd, const char* data, size_t data_size) override {
    ...
    if (!st.want_write) { st.want_write = true; submit_send(fd); }
    return true;
  }

  bool post(uint32_t value) override {
    if (user_efd_ == -1) return false;
  ...
  bool set_read_timeout(socket_t socket, uint32_t timeout_ms) override {
    ...
    if (timeout_ms == 0) {
      if (st.timeout_armed) { submit_timeout_remove(socket, st.timeout_key); st.timeout_armed = false; st.timeout_key = 0; }
      return true;
    }
    // arm or re-arm
    if (st.timeout_armed) { submit_timeout_remove(socket, st.timeout_key); st.timeout_armed = false; st.timeout_key = 0; }
...
private:
  struct SockState {
    char* buf{nullptr}; size_t buf_size{0}; ReadCallback read_cb; std::vector<char> out_queue; bool want_write{false}; bool connecting{false};
    bool paused{false}; // added paused flag
    uint32_t timeout_ms{0}; bool timeout_armed{false}; uint64_t timeout_key{0};
  };
      ...
      if (ud) {
        switch (ud->op) {
          case Op::User: {
            ...
            uint64_t cnt = 0; while (::read(user_efd_, &cnt, sizeof(cnt)) == sizeof(cnt)) { if (cbs_.on_user) cbs_.on_user((uint32_t)cnt); }
            submit_read_user();
            break;
          }
          case Op::Accept: {
            ...
            if (res >= 0) {
              socket_t client = res;
              if (max_conn_ > 0 && cur_conn_.load() >= max_conn_) { ::close(client); }
              else {
          ...
          case Op::Read: {
            ...
            if (it != sockets_.end()) {
              ...
              if (res > 0) {
                auto cb = st.read_cb; auto buf = st.buf; size_t n = (size_t)res; lk.unlock(); if (cb) cb(fd, buf, n); lk.lock();
                // submit next read
                submit_recv(fd);
                ...
                if (it2 != sockets_.end()) {
                  auto& st2 = it2->second;
                  if (st2.timeout_ms > 0) {
                    if (st2.timeout_armed) { submit_timeout_remove(fd, st2.timeout_key); st2.timeout_armed = false; st2.timeout_key = 0; }
                ...
                if (st.timeout_armed) { submit_timeout_remove(fd, st.timeout_key); }
                lk.unlock(); IO_LOG_DBG("close: fd=%d", (int)fd); if (cbs_.on_close) cbs_.on_close(fd);
          ...
          case Op::Write: {
            ...
            if (it != sockets_.end()) {
              auto& st = it->second;
              if (res > 0) {
                size_t wrote = (size_t)res; if (cbs_.on_write) { lk.unlock(); cbs_.on_write(fd, wrote); lk.lock(); }
                if (wrote <= st.out_queue.size()) st.out_queue.erase(st.out_queue.begin(), st.out_queue.begin() + wrote);
              }
              if (!st.out_queue.empty()) { submit_send(fd); }
              else { st.want_write = false; }
            }
            break;
          }
          case Op::Connect: {
            ...
            auto it = sockets_.find(fd);
            if (it != sockets_.end()) {
              auto& st = it->second; st.connecting = false;
              if (ud->extra == 2) {
                // native connect: res==0 success, negative error otherwise
                if (res == 0) { lk.unlock(); IO_LOG_DBG("connect: established fd=%d", (int)fd); if (cbs_.on_accept) cbs_.on_accept(fd); submit_recv(fd); }
                else {
                  int e = -res; if (e < 0) e = errno; // fallback
                  IO_LOG_ERR("connect completion fd=%d failed errno=%d (%s)", (int)fd, e, std::strerror(e));
                  lk.unlock(); if (cbs_.on_close) cbs_.on_close(fd); ::close(fd); std::lock_guard<std::mutex> lk2(mtx_); sockets_.erase(fd);
                }
              } else {
                ...
                if (res == 0) {
                  ...
                  if (gs < 0) { int e = errno; IO_LOG_ERR("getsockopt(SO_ERROR fd=%d) failed errno=%d (%s)", (int)fd, e, std::strerror(e)); err = e; }
                  else { IO_LOG_DBG("connect completion fd=%d, SO_ERROR=%d (%s)", (int)fd, err, std::strerror(err)); }
                  if (err == 0) { lk.unlock(); IO_LOG_DBG("connect: established fd=%d", (int)fd); if (cbs_.on_accept) cbs_.on_accept(fd); submit_recv(fd); }
                  else { lk.unlock(); if (cbs_.on_close) cbs_.on_close(fd); ::close(fd); std::lock_guard<std::mutex> lk2(mtx_); sockets_.erase(fd); }
                } else { lk.unlock(); if (cbs_.on_close) cbs_.on_close(fd); ::close(fd); std::lock_guard<std::mutex> lk2(mtx_); sockets_.erase(fd); }
              }
            }
            if (ud->ptr) { delete static_cast<sockaddr_in*>(ud->ptr); }
            break;
          }
          case Op::Timeout: {
            ...
            socket_t fd = (socket_t)-1;
            {
              ...
              if (itf != timeouts_fd_.end()) { fd = itf->second; timeouts_fd_.erase(itf); }
              timeouts_ud_.erase(key);
              auto itS = sockets_.find(fd);
              if (itS != sockets_.end()) { itS->second.timeout_armed = false; itS->second.timeout_key = 0; }
            }
            if (res == -ETIME && fd != -1) {
          ...
          case Op::TimeoutRemove: {
            ...
            uint64_t key = ud->u64;
            UringData* tud = nullptr; socket_t fd=(socket_t)-1;
            {
              std::lock_guard<std::mutex> lk(mtx_);
              auto itu = timeouts_ud_.find(key);
              if (itu != timeouts_ud_.end()) { tud = itu->second; timeouts_ud_.erase(itu); }
              auto itf = timeouts_fd_.find(key);
              if (itf != timeouts_fd_.end()) { fd = itf->second; timeouts_fd_.erase(itf); }
              if (fd != -1) { auto itS = sockets_.find(fd); if (itS != sockets_.end()) { itS->second.timeout_armed = false; itS->second.timeout_key = 0; } }
            }
            if (tud) delete tud;
            break;
          }
  ...
  bool pause_read(socket_t fd) override {
    std::lock_guard<std::mutex> lk(mtx_);
    auto it = sockets_.find(fd); if (it == sockets_.end()) return false; it->second.paused = true; return true;
  }
  bool resume_read(socket_t fd) override {
    {
      std::lock_guard<std::mutex> lk(mtx_);
      auto it = sockets_.find(fd); if (it == sockets_.end()) return false; it->second.paused = false;
    }
    submit_recv(fd);
    return true;
  }
  ...
  void submit_recv(socket_t fd) {
    ...
    if (io_uring_submit(&ring_) < 0) { IO_LOG_ERR("io_uring_submit(recv fd=%d) failed", (int)fd); }
  }
  ...
  void submit_send(socket_t fd) {
    ...
    io_uring_sqe_set_data(sqe, ud);
    if (io_uring_submit(&ring_) < 0) { IO_LOG_ERR("io_uring_submit(send fd=%d) failed", (int)fd); }
  }
  ...
  void submit_pollout(socket_t fd) {
    ...
    io_uring_sqe_set_data(sqe, ud);
    if (io_uring_submit(&ring_) < 0) { IO_LOG_ERR("io_uring_submit(pollout fd=%d) failed", (int)fd); }
  }
  ...
  void submit_connect(socket_t fd, const sockaddr_in& addr) {
    ...
    io_uring_sqe_set_data(sqe, ud);
    if (io_uring_submit(&ring_) < 0) { IO_LOG_ERR("io_uring_submit(connect fd=%d) failed", (int)fd); }
  }
  ...
  void submit_read_user() {
    ...
    io_uring_sqe_set_data(sqe, ud);
    if (io_uring_submit(&ring_) < 0) { IO_LOG_ERR("io_uring_submit(user read) failed"); }
  }
  ...
  void submit_timeout(socket_t fd, uint32_t ms) {
    ...
    {
      std::lock_guard<std::mutex> lk(mtx_);
      auto it = sockets_.find(fd); if (it == sockets_.end()) { delete ud; return; }
      it->second.timeout_armed = true; it->second.timeout_key = key;
      timeouts_ud_[key] = ud;
      timeouts_fd_[key] = fd;
    }
    ...
    io_uring_sqe_set_data(sqe, ud);
    if (io_uring_submit(&ring_) < 0) { IO_LOG_ERR("io_uring_submit(timeout fd=%d) failed", (int)fd); }
  }
  ...
  void submit_timeout_remove(socket_t /*fd*/, uint64_t key) {
    ...
    io_uring_sqe_set_data(sqe, ud);
    if (io_uring_submit(&ring_) < 0) { IO_LOG_ERR("io_uring_submit(timeout_remove key=%llu) failed", (unsigned long long)key); }
  }
