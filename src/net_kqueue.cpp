...
bool KqueueEngine::post(uint32_t user_event_value) {
	...
	{
		std::lock_guard<std::mutex> lk(userq_mtx_);
...
void KqueueEngine::event_loop() {
	...
	while (running_) {
		...
		int nev = kevent(kq_, nullptr, 0, events.data(), MAX_EVENTS, &ts);
		if (nev < 0) {
			if (errno == EINTR)
				continue;
		...
		for (int i = 0; i < nev; ++i) {
			...
			if (ev.filter == EVFILT_TIMER) {
				...
				{
					std::lock_guard<std::mutex> lk(mtx_);
					exists = sockets_.find(fd_to_close) != sockets_.end();
				}
				if (exists) {
					if (cbs_.on_close)
			...
			if (ev.filter == EVFILT_READ) {
				...
				if (st.connecting) {
					...
					if (err == 0) {
						...
						lk.unlock();
						if (cbs_.on_close)
							cbs_.on_close(saved_fd);
						::close(saved_fd);
						struct kevent dv{};
						EV_SET(&dv, saved_fd, EVFILT_TIMER, EV_DELETE, 0, 0, nullptr);
						(void)kevent(kq_, &dv, 1, nullptr, 0, nullptr);
						std::lock_guard<std::mutex> lk2(mtx_);
						sockets_.erase(saved_fd);
						timeouts_ms_.erase(saved_fd);
					}
					continue;
				}
