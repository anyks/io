  ...
  public:
	...
	bool accept(socket_t listen_socket, bool async, uint32_t max_connections) override {
		...
		if (::port_associate(port_, PORT_SOURCE_FD, listen_socket, POLLIN, nullptr) != 0) {
			IO_LOG_ERR("port_associate(listen=%d, POLLIN) failed errno=%d (%s)", (int)listen_socket, errno,
					   std::strerror(errno));
			return false;
		}
		return true;
	...
	bool set_read_timeout(socket_t socket, uint32_t timeout_ms) override {
		...
		if (timer_settime(tid, 0, &its, nullptr) != 0) {
			IO_LOG_ERR("timer_settime failed errno=%d (%s)", errno, std::strerror(errno));
			return false;
		}
		return true;
	}
  ...
  private:
	...
	void event_loop() {
		while (running_) {
			...
			uint_t nget = 1;
			timespec ts{1, 0};
			int r = ::port_getn(port_, evs, 64, &nget, &ts);
			if (r != 0) {
				if (errno == EINTR)
					continue;
				...
				if (ev.portev_source == PORT_SOURCE_FD) {
					...
					if (ev.portev_events & POLLOUT) {
						...
						if (it != sockets_.end() && it->second.connecting) {
							...
							if (gs < 0) {
								err = errno;
								IO_LOG_ERR("getsockopt(SO_ERROR fd=%d) failed errno=%d (%s)", fd, err,
										   std::strerror(err));
							} else {
