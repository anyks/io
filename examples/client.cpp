...
static inline uint32_t be32_read(const char* in) {
  uint32_t n = 0; std::memcpy(&n, in, sizeof(n));
#if defined(_WIN32) || defined(_WIN64)
  return ntohl(n);
#else
  return ntohl(n);
...
int main(){
  ...
  for (int i = 1; i <= 100; ++i) {
    ...
    if (sz == 0) sz = 1;
    std::string m; m.resize(sz);
    for (size_t j = 0; j < sz; ++j) m[j] = char('A' + ((i + int(j)) % 26));
    messages.emplace_back(std::move(m));
  }

  // Receive-side state
  std::vector<char> recv_accum; recv_accum.reserve(1 << 16);
  ...
  cbs.on_close = [&](socket_t s){ std::cout << "closed: 0x" << std::hex << (static_cast<std::uintptr_t>(s)) << std::dec << std::endl; };

  if (!engine->init(cbs)) { std::cerr << "init failed" << std::endl; return 1; }

  socket_t fd = ::socket(AF_INET, SOCK_STREAM, 0);
  if (fd == kInvalidSocket) { std::cerr << "socket() failed" << std::endl; return 1; }
  ...
  engine->add_socket(fd, read_buf, sizeof(read_buf), [&](socket_t /*s*/, char* b, size_t n){
    ...
    size_t offset = 0;
    while (true) {
      if (recv_accum.size() - offset < 4) break;
      uint32_t len = be32_read(&recv_accum[offset]);
      if (recv_accum.size() - offset < 4u + len) break;
      ...
      if (idx < messages.size()) {
        ...
        if (!ok) {
          ...
          done.store(true); exit_code.store(2);
          break;
        }
      }
      frames_received++;
      ...
      if (frames_received == messages.size()) {
        ...
        done.store(true); exit_code.store(0);
      }
    }
    if (offset > 0) {
  ...
  if (!engine->connect(fd, "127.0.0.1", 8080, true)) { std::cerr << "connect failed" << std::endl; return 1; }

  // Wait for connection notification before sending
  for (int i=0; i<200 && !connected.load(); ++i) std::this_thread::sleep_for(std::chrono::milliseconds(5));
  if (!connected.load()) { std::cerr << "connect timeout" << std::endl; return 1; }

  // Send frames: [len][payload]
  for (const auto& m : messages) {
    char hdr[4]; be32_write(static_cast<uint32_t>(m.size()), hdr);
    engine->write(fd, hdr, sizeof(hdr));
    if (!m.empty()) engine->write(fd, m.data(), m.size());
  }
