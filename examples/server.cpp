...
int main() {
	...
	if (!engine) {
		std::cerr << "No engine available for this platform" << std::endl;
		return 1;
	}
	...
	cbs.on_accept = [&](socket_t s) {
		...
		engine->add_socket(s, readbufs[s].data(), readbufs[s].size(), [&](socket_t fd, char *b, size_t n) {
			auto &acc = accum[fd];
			acc.insert(acc.end(), b, b + n);
			size_t off = 0;
			while (true) {
				if (acc.size() - off < 4)
					break;
				uint32_t len = be32_read(&acc[off]);
				if (acc.size() - off < 4u + len)
					break;
			...
			if (off > 0)
				acc.erase(acc.begin(), acc.begin() + static_cast<std::ptrdiff_t>(off));
		});
