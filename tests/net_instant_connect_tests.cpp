...
TEST(NetInstantConnect, AddSocketBeforeConnect_BufferAndCallbackSurvive) {
  ...
  std::promise<void> delivered_promise; auto delivered_future = delivered_promise.get_future();
  ...
  cbs.on_accept = [&](int fd){
    ...
    if (fd == listen_fd) return; // игнорируем сам listen
    // Принятый сервером сокет
    server_accepted = true;
    server_fd = fd;
    // Назначим обработчик (хоть и не обязателен — будем только писать)
    engine->add_socket(fd, server_buf, sizeof(server_buf), [&](int, char*, size_t){ /*noop*/ });
  ...
  cbs.on_read = [&](int s, char* b, size_t n){
    ...
    if (n >= payload.size() && std::string(b, b + payload.size()) == payload) {
      if (!delivered_set.exchange(true)) delivered_promise.set_value();
    }
  };
  ...
  int sfd = server_fd.load(); if (sfd != -1) engine->disconnect(sfd);
  ::close(listen_fd);
  engine->destroy();
}
