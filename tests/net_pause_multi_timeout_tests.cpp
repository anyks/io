...
TEST(NetPauseMultiTimeout, MixedResumeAndTimeouts) {
  ...
  constexpr int N = 8;
  std::vector<std::atomic<int>> client_fds(N);
  std::vector<char> client_bufs(N * 1024);
  std::vector<std::atomic<int>> delivered(N);
  for (int i=0;i<N;++i) { delivered[i] = 0; client_fds[i] = -1; }
  ...
  cbs.on_accept = [&](int fd){
    if (fd == listen_fd) return; // игнорируем сам listen сокет на всякий случай
    ...
    for (int i = 0; i < N; ++i) { if (client_fds[i].load() == fd) { is_client_socket = true; break; } }
    if (is_client_socket) return;
    // Принятый сервером сокет: назначаем чтение и эхо, учитываем приёмы
    INetEngine* eng = engine.get();
    engine->add_socket(fd, server_buf, sizeof(server_buf), [eng](int s, char* b, size_t n){ if (n>0) eng->write(s,b,n); });
    server_accepts++;
  };
  cbs.on_read = [&](int s, char* b, size_t n){
    ...
    for (int i=0;i<N;++i) {
      if (client_fds[i].load() == s) { delivered[i] += (int)n; break; }
    }
  };
  ...
  for (int i=0;i<N;++i) {
    int cfd = ::socket(AF_INET, SOCK_STREAM, 0); ASSERT_GE(cfd, 0); client_fds[i] = cfd;
  ...
  while (server_accepts.load() < N) {
    if (std::chrono::steady_clock::now() - tstart > 2s) break;
  ...
  while (true) {
    int ok=0; for (int i=0;i<N/2;++i) if (delivered[i].load() >= (int)payload.size()) ok++;
    if (ok == N/2) break;
    if (std::chrono::steady_clock::now() - t0 > 2s) break;
    std::this_thread::sleep_for(5ms);
  }
  for (int i=0;i<N/2;++i) EXPECT_EQ(delivered[i].load(), (int)payload.size());
  ...
  // cleanup
  for (int i=0;i<N;++i) engine->disconnect(client_fds[i]);
  ::close(listen_fd);
  engine->destroy();
}
