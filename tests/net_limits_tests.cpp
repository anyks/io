...
namespace {
static int make_listen(uint16_t& out_port) {
  ...
  ::listen(fd, 256);
  socklen_t alen=sizeof(addr); ::getsockname(fd, (sockaddr*)&addr, &alen);
...
TEST(NetLimits, RandomChurnConnectDisconnect) {
  ...
  auto srv_bufs = std::make_shared<std::unordered_map<int, std::unique_ptr<char[]>>>();
  std::unordered_set<int> client_set; std::mutex client_mtx;
  ...
  cbs.on_accept = [&](int fd){
    ...
    if (client_set.count(fd) == 0) {
      accepts++;
      auto buf = std::make_unique<char[]>(1024); char* raw = buf.get();
      { std::lock_guard<std::mutex> ls(*srv_mtx); srv_bufs->emplace(fd, std::move(buf)); }
      engine->add_socket(fd, raw, 1024, [&, fd](int, char* b, size_t n){ engine->write(fd, b, n); });
    }
  };
  cbs.on_close = [srv_mtx, srv_bufs, &closes](int fd){ closes++; std::lock_guard<std::mutex> ls(*srv_mtx); srv_bufs->erase(fd); };

  ASSERT_TRUE(engine->init(cbs));

  uint16_t port=0; int listen_fd = make_listen(port);
  ...
  for (int i=0;i<iterations;++i) {
    ...
    ASSERT_GE(cfd, 0);
    {
      std::lock_guard<std::mutex> lk(client_mtx); client_set.insert(cfd);
    }
    char buf[512];
    ASSERT_TRUE(engine->add_socket(cfd, buf, sizeof(buf), [&](int, char*, size_t){ /* ignore */ }));
    ...
    if (!connected) {
      for (int r=0;r<3 && !connected;++r) { std::this_thread::sleep_for(1ms); connected = engine->connect(cfd, "127.0.0.1", port, true); }
    }
...
TEST(NetLimits, EnforceMaxConnections) {
  ...
  std::unordered_set<int> client_fds; std::mutex cm;
  NetCallbacks cbs{};
  cbs.on_accept = [&](int fd){ std::lock_guard<std::mutex> lk(cm); if (client_fds.count(fd)==0) server_accepts++; };
  ASSERT_TRUE(engine->init(cbs));
  ...
  for (uint32_t i=0;i<maxC;++i) {
    ...
    { std::lock_guard<std::mutex> lk(cm); client_fds.insert(fd); }
    char buf[256]; engine->add_socket(fd, buf, sizeof(buf), nullptr);
  ...
  for (int fd : wave1) engine->disconnect(fd);
  ...
  for (int fd : wave2) engine->disconnect(fd);
  ::close(listen_fd);
  engine->destroy();
}
