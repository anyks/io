...
TEST(NetPauseResumeServer, ServerPauseSuppressesThenResumeDelivers) {
	...
	cbs.on_accept = [&](io::socket_t fd) {
		if (fd == client_fd) {
			...
			engine->add_socket(fd, server_buf, sizeof(server_buf), [&](io::socket_t s, char *b, size_t n) {
				(void)s;
				(void)b;
				server_bytes += (int)n;
			});
	...
	cbs.on_read = [&](io::socket_t, char *, size_t) {};
	...
	ASSERT_NE(server_fd.load(), io::kInvalidSocket);
