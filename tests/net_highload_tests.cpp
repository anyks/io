...
TEST(NetHighload, ManyClientsEchoNoBlock) {
  ...
  if (const char* s = std::getenv("IO_STRESS_M")) { int v = std::atoi(s); if (v > 0) M = v; }

  // Bound N by file descriptor soft limit to prevent exhaustion (approx. 2 fds per client + listen)
  rlimit rl{}; if (getrlimit(RLIMIT_NOFILE, &rl) == 0) {
    long max_fd = (long)rl.rlim_cur; if (max_fd <= 128) max_fd = 128; long budget = max_fd - 64; if (budget < 64) budget = 64; long max_clients = budget / 2; if (N > max_clients) N = (int)max_clients; if (N < 1) N = 1;
  }
  ...
  // before the containers captured by callbacks, even on early EXPECT failures.
  {
    ...
    cbs.on_accept = [&](int fd){
      ...
      if (fd != listen_fd && !is_client_fd) {
        ...
        engine->add_socket(fd, raw, 4096, [&, fd](int s, char* b, size_t n){ (void)s; engine->write(fd, b, n); });
      } else {
        ...
        int idx = -1;
        {
          ...
          if (it != fd_to_index.end()) idx = it->second;
        }
  ...
  // Create clients
    ...
    ASSERT_TRUE(engine->add_socket(fd, clients[i].buf.get(), 2048, [&, i](int s, char* b, size_t n){ (void)s; (void)b; (void)n; if (!clients[i].echoed.exchange(true)) echoed_clients++; }));
  }
    ...
    while (std::chrono::steady_clock::now() < deadline && !ok) {
      ...
      if (!ok) {
        ...
        if (attempts % 5 == 0) {
          ...
          int badfd = clients[i].fd;
          engine->delete_socket(badfd);
          ::close(badfd);
          int newfd = ::socket(AF_INET, SOCK_STREAM, 0);
          if (newfd < 0) { std::this_thread::sleep_for(5ms); continue; }
          clients[i].fd = newfd;
          clients[i].buf = std::make_unique<char[]>(2048);
          {
            std::lock_guard<std::mutex> lk(client_fds_mtx);
          ...
          engine->add_socket(newfd, clients[i].buf.get(), 2048, [&, i](int s, char* b, size_t n){ (void)s; (void)b; (void)n; if (!clients[i].echoed.exchange(true)) echoed_clients++; });
        }
      } else {
        // Wait a bit for the async connect to complete (on_accept) or fail (on_close)
        auto wait_until = std::chrono::steady_clock::now() + 200ms;
        while (std::chrono::steady_clock::now() < wait_until && !clients[i].connected.load(std::memory_order_relaxed) && !clients[i].closed.load(std::memory_order_relaxed)) {
  ...
  for (int i=0;i<M;++i) engine->post((uint32_t)(i+1));

  // Send data from each client
  const char* payload = "x";
  for (int i=0;i<N;++i) ASSERT_TRUE(engine->write(clients[i].fd, payload, 1));
  ...
  while (echoed_clients.load() < N || user_count.load() < (uint64_t)M) {
    if (std::chrono::steady_clock::now() - t1 > 15s) {
      ADD_FAILURE() << "Timeout: echoed=" << echoed_clients.load() << "/" << N << ", user_count=" << user_count.load() << "/" << M;
      break;
    }
    std::this_thread::sleep_for(5ms);
  }
