...
TEST(NetPauseResume, PauseSuppressesThenResumeDelivers) {
  ...
  std::promise<void> got_after_resume; auto fut_after_resume = got_after_resume.get_future();
  ...
  cbs.on_accept = [&](int fd){
    if (fd == client_fd) {
      ...
      engine->add_socket(fd, server_buf, sizeof(server_buf), [eng, &server_reads](int s, char* b, size_t n){
        (void)eng; server_reads++;
        if (n > 0) {
          eng->write(s, b, n);
        }
      });
  ...
  cbs.on_read = [&](int s, char* b, size_t n){
    (void)s; (void)b; delivered_bytes += (int)n; client_reads++;
  };
  ...
  while (!(client_connected.load() && server_connected.load())) {
    if (std::chrono::steady_clock::now() - start > 2s) FAIL() << "Timeout waiting for connections";
