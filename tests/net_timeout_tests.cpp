...
TEST(NetTimeout, ReadIdleCloseClient) {
  ...
  std::atomic<int> server_client_fd{-1};
  int client_fd = ::socket(AF_INET, SOCK_STREAM, 0);
  ASSERT_GE(client_fd, 0);

  io::NetCallbacks cbs{};
  cbs.on_accept = [&](int fd){ if (fd != listen_fd) { server_connected = true; server_client_fd.store(fd, std::memory_order_relaxed); } };
  ...
  // Client side
  std::promise<void> conn_p; auto conn_f = conn_p.get_future();
  ...
  while (!server_connected.load()) { if (std::chrono::steady_clock::now() - start > 2s) { FAIL() << "Timeout waiting for server accept"; break; } std::this_thread::sleep_for(10ms); }
  ...
  while (close_count.load() == 0) { if (std::chrono::steady_clock::now() - start > 2s) break; std::this_thread::sleep_for(10ms); }
