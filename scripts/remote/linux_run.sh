...
echo "[linux] install deps (might require sudo password)"
remote "sudo apt-get update && sudo apt-get install -y cmake ninja-build g++ liburing-dev || true"

echo "[linux][epoll] configure/build/test"
remote "set -e; cd '$LINUX_DIR'; cmake -S . -B build/epoll -G Ninja -DIO_BUILD_TESTS=ON -DIO_WITH_IOURING=OFF -DCMAKE_BUILD_TYPE=Debug; cmake --build build/epoll -j; ctest --test-dir build/epoll -j4 --output-on-failure || true"
...
echo "[linux][iouring] configure/build/test"
remote "set -e; cd '$LINUX_DIR'; cmake -S . -B build/iouring -G Ninja -DIO_BUILD_TESTS=ON -DIO_WITH_IOURING=ON -DCMAKE_BUILD_TYPE=Debug; cmake --build build/iouring -j; ctest --test-dir build/iouring -j4 --output-on-failure || true"
