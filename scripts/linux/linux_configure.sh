...
ssh -p "${LINUX_SSH_PORT}" ${LINUX_SSH_OPTS} "${LINUX_SSH}" bash -s -- \
  ...
  "${LINUX_CMAKE}" \
  "${LINUX_BUILD_TYPE}" \
  "${LINUX_IOURING}" <<'REMOTE_SCRIPT'
set -euo pipefail
LINUX_DIR="$1"; LINUX_BUILD="$2"; LINUX_CMAKE="$3"; LINUX_BUILD_TYPE="$4"; LINUX_IOURING="$5"
...
echo "Configured on ${LINUX_SSH} in ${LINUX_DIR}/${LINUX_BUILD} (IO_URING=${LINUX_IOURING})"
