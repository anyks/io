...
set -euo pipefail
LINUX_DIR="$1"; LINUX_BUILD="$2"; LINUX_CTEST="$3"; LINUX_CTEST_TIMEOUT="$4"
cd "$LINUX_DIR"
LOG="$LINUX_BUILD/ctest_last.log"
echo "[remote] Running ctest in $LINUX_BUILD. Log: $LOG"
set +e
"$LINUX_CTEST" --test-dir "$LINUX_BUILD" --output-on-failure -j2 --timeout "$LINUX_CTEST_TIMEOUT" >"$LOG" 2>&1
CTEST_RC=$?
set -e
